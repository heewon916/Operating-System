...
static void print_result(double ins_time, double del_time, double tot_time, int lock_type, int num_threads, int node_count) {
	...
	if (lock_type == 0) { // no lock
	...
	} else if (lock_type == 1) { // coarse grained lock
		for (int i = 0; i < 2; i++) { 
			printf("\n=====  Multi Threads %s  %s experiment =====\n", cond[lock_type], op[i]);  	   
			printf("\n Experiment Info \n");
			printf("    Test Node Count         : %d \n", node_count);
        ...
        printf("\n  Total Execution Time        : %.2lf seconds \n", time[2]);
	} else if (lock_type == 2) { // fine grained lock 
		for (int i = 0; i < 2; i++) {
			printf("\n===== Multi Threads %s  %s experiment =====\n", cond[lock_type], op[i]);
			printf("\n Experiment Info \n");
			printf("    Test Node Count         : %d \n", node_count);
...
void* thread_job_insert(void *arg) {	// thread inserts the job
	thread_arg *th_arg = (thread_arg *)arg; 
	int node_c = th_arg->node_count;	// the number of total node that all thread try to insert
	int thread_n = th_arg->thread_num; 	// the number of total thread

	for(int i = 0; i < node_c/thread_n; i++) { 	// the number of data a thread try to insert
		target = rand() % node_c;	// insert the random num modulered; target is the num should be inserted actually
		hash_queue_insert_by_target(); 	// the target will inserted in the hash-queue 
	}
}
...
int main(int argc, char *argv[]) {
	...
	for (int i = 1; i < argc; i++) {
		if (sscanf(argv[i], "-t=%d%c", &n, &junk) == 1) { // store argv[i][0] in n, argv[i][1] in junk
			num_threads = n;
		}
